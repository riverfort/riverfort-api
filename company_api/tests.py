...
class CompanyProfileTests(APITestCase):
    ...
    def test_view_company_trading_list(self):
        url = reverse('company_api:company-trading-list', kwargs={'pk': 'testing-company-ticker'})
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_view_company_adtv_list(self):
        url = reverse('company_api:company-adtv-list', kwargs={'pk': 'testing-company-ticker'})
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_view_company_adtv_quote(self):
        url = reverse('company_api:company-adtv-quote', kwargs={'pk': 'testing-company-ticker'})
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_view_company_trading_quote(self):
        url = reverse('company_api:company-trading-quote', kwargs={'pk': 'testing-company-ticker'})
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_view_company_quote_full(self):
        url = reverse('company_api:company-quote-full', kwargs={'ticker': 'testing-company-ticker'})
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
